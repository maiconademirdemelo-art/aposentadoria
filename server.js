...
async function initDB() {
  try {
    ...
    // Adiciona as colunas LGPD se ainda não existem (para bases antigas)
    await pool.query(`ALTER TABLE cooperados ADD COLUMN IF NOT EXISTS data_consentimento TIMESTAMP`);
    await pool.query(`ALTER TABLE cooperados ADD COLUMN IF NOT EXISTS consentimento_revogado_em TIMESTAMP`);
    
    // Tabela de log de exclusões LGPD (para efeitos legais)
    await pool.query(`
      CREATE TABLE IF NOT EXISTS lgpd_exclusoes (
        id SERIAL PRIMARY KEY,
        celular_hash TEXT NOT NULL,
        nome_parcial TEXT,
        excluido_em TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        motivo TEXT DEFAULT 'solicitacao_titular'
      )
    `);
...
console.log('🔑 ANTHROPIC_API_KEY configurado:', process.env.ANTHROPIC_API_KEY ? 'SIM (' + process.env.ANTHROPIC_API_KEY.substring(0,12) + '...)' : '❌ NÃO — módulo Análise de Ações não vai funcionar');
...
app.post('/api/salvar', async (req, res) => {
  ...
  const {
    nome, celular, fluxo, data_consentimento,
  ...
  try {
    ...
    if (existing.rows.length > 0) {
      ...
      // Primeira gravação: registra também a data do consentimento LGPD
      const dataConsent = data_consentimento || new Date().toISOString();
      const result = await pool.query(
        'INSERT INTO cooperados (nome, celular, data_consentimento) VALUES ($1, $2, $3) RETURNING id',
        [nome, celular || '', dataConsent]
      );
      cooperado = result.rows[0];
      console.log('🆕 Novo cadastro criado com consentimento LGPD em', dataConsent);
    }
...
// O titular pode solicitar a exclusão completa dos seus dados
// conforme Art. 18, VI da LGPD
app.post('/api/meus-dados/excluir', async (req, res) => {
  const { celular } = req.body;
  
  if (!celular || typeof celular !== 'string' || celular.length < 10) {
    return res.status(400).json({ success: false, error: 'Celular inválido' });
  }
  
  // Normaliza celular (remove não-dígitos)
  const celNorm = celular.replace(/\D/g, '');
  
  try {
    // Busca todos os cooperados com esse celular
    const found = await pool.query(
      `SELECT id, nome FROM cooperados WHERE regexp_replace(celular, '[^0-9]', '', 'g') = $1`,
      [celNorm]
    );
    
    if (found.rows.length === 0) {
      return res.status(404).json({ success: false, error: 'Nenhum registro encontrado com este celular' });
    }
    
    // Registra log de exclusão (com hash para não guardar celular cru)
    const crypto = require('crypto');
    const celHash = crypto.createHash('sha256').update(celNorm).digest('hex');
    
    for (const row of found.rows) {
      const nomeParcial = row.nome ? row.nome.substring(0, 3) + '***' : '***';
      await pool.query(
        'INSERT INTO lgpd_exclusoes (celular_hash, nome_parcial, motivo) VALUES ($1, $2, $3)',
        [celHash, nomeParcial, 'solicitacao_titular']
      );
      
      // Exclui o cooperado (CASCADE remove todos os dados relacionados)
      await pool.query('DELETE FROM cooperados WHERE id = $1', [row.id]);
      console.log(`🗑 LGPD: Dados excluídos de ${nomeParcial} (id ${row.id})`);
    }
    
    res.json({ 
      success: true, 
      registros_excluidos: found.rows.length,
      mensagem: 'Dados excluídos com sucesso. Confirmação registrada para efeitos legais.'
    });
  } catch (err) {
...
// ── Revogar consentimento (sem excluir dados) ──
// Titular mantém os dados mas interrompe o tratamento ativo
app.post('/api/meus-dados/revogar-consentimento', async (req, res) => {
  const { celular } = req.body;
  if (!celular) return res.status(400).json({ success: false, error: 'Celular obrigatório' });
  
  const celNorm = celular.replace(/\D/g, '');
  try {
    const result = await pool.query(
      `UPDATE cooperados SET consentimento_revogado_em = CURRENT_TIMESTAMP 
       WHERE regexp_replace(celular, '[^0-9]', '', 'g') = $1 
       RETURNING id`,
      [celNorm]
    );
    res.json({ success: true, registros_atualizados: result.rows.length });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
});
...
// ══════════════════════════════════════════════════════════════
// MÓDULO ANÁLISE DE AÇÕES — proxy para Anthropic API
// ══════════════════════════════════════════════════════════════
app.post('/api/analise-acoes/messages', async (req, res) => {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) {
    console.error('[analise-acoes] ❌ ANTHROPIC_API_KEY não encontrada. Variáveis disponíveis:', Object.keys(process.env).filter(k => !k.startsWith('npm_')).join(', '));
    return res.status(500).json({ error: 'ANTHROPIC_API_KEY não configurada', hint: 'A variável não existe no ambiente do servidor. Verifique no Railway: Variables → ANTHROPIC_API_KEY' });
  }
  ...
  const safeBody = {
    ...
    max_tokens: Math.min(Math.max(parseInt(max_tokens) || 2000, 100), 4000),
    messages: messages.slice(0, 10)
  };
  if (system) safeBody.system = String(system).slice(0, 4000);
  if (tools && Array.isArray(tools)) safeBody.tools = tools;
  try {
    ...
    console.error('[analise-acoes] erro:', err);
    res.status(502).json({ error: 'Falha ao chamar a Anthropic API', detail: String(err.message || err) });
  }
});
