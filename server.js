...
function getAuth(){
  const fromUrl = new URLSearchParams(window.location.search).get('auth');
  if(fromUrl){ localStorage.setItem('wpAuth', fromUrl); return fromUrl; }
  return localStorage.getItem('wpAuth') || '';
}
...
async function carregar(){
  ...
  try{
    ...
    if(!r.ok) throw new Error('Erro '+r.status+' — verifique o login');
    dados = (await r.json()).sort((a,b) => Number(b.patrimonio_futuro) - Number(a.patrimonio_futuro));
    renderStats();
    renderLista(dados);
  }catch(e){
    document.getElementById('list').innerHTML = '<div class="empty"><div class="empty-icon">⚠️</div>'+e.message+'</div>';
  }
}
