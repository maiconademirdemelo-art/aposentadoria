  ...
  try {
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS diagnosticos_aposentadoria (
        ...
        perfil TEXT CHECK (perfil IN ('conservador', 'moderado', 'arrojado')),
        estrategia TEXT CHECK (estrategia IN ('renda', 'consumo')),
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS simulacoes_holding (
        ...
        recomendacao TEXT CHECK (recomendacao IN ('holding', 'pf')),
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
      )
    `);
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS simulacoes_retirada (
        id SERIAL PRIMARY KEY,
        diagnostico_id INTEGER REFERENCES diagnosticos_aposentadoria(id) ON DELETE CASCADE,
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS sessoes (
        id SERIAL PRIMARY KEY,
        cooperado_id INTEGER REFERENCES cooperados(id) ON DELETE CASCADE,
        fluxo TEXT CHECK (fluxo IN ('raioX', 'pontual')),
        modulos_visitados TEXT[], -- array de strings: aposentadoria, pgbl, holding
    ...
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_cooperados_nome ON cooperados(nome)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_cooperados_celular ON cooperados(celular)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_diagnosticos_cooperado ON diagnosticos_aposentadoria(cooperado_id)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_sessoes_cooperado ON sessoes(cooperado_id)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_sessoes_created ON sessoes(created_at DESC)`);
...
// ── AUTENTICAÇÃO ADMIN ──
const ADMIN_USER = process.env.ADMIN_USER || 'maiconademirdemelo@gmail.com';
const ADMIN_PASS = process.env.ADMIN_PASS || 'Unicred@1943';
...
app.get('/admin/api/stats', authMiddleware, async (req, res) => {
  try {
    const stats = await pool.query(`
      SELECT 
        (SELECT COUNT(*) FROM cooperados) as total_cooperados,
        (SELECT COUNT(*) FROM diagnosticos_aposentadoria) as total_diagnosticos,
        (SELECT COUNT(*) FROM simulacoes_pgbl) as total_pgbl,
        (SELECT COUNT(*) FROM simulacoes_holding) as total_holding,
        (SELECT COUNT(*) FROM sessoes WHERE fluxo = 'raioX') as total_raio_x,
        (SELECT COUNT(*) FROM sessoes WHERE fluxo = 'pontual') as total_pontual,
        (SELECT COUNT(*) FROM sessoes WHERE completou_raio_x = TRUE) as raio_x_completos,
        (SELECT COUNT(*) FROM cooperados WHERE created_at > NOW() - INTERVAL '7 days') as novos_7_dias,
        (SELECT AVG(patrimonio_futuro) FROM diagnosticos_aposentadoria) as media_patrimonio_futuro,
        (SELECT AVG(renda_mensal) FROM diagnosticos_aposentadoria) as media_renda_projetada,
        (SELECT COUNT(*) FROM dados_renda WHERE renda_em_folha = TRUE) as total_renda_folha,
        (SELECT COUNT(*) FROM dados_imoveis WHERE possui_imoveis = TRUE) as total_possui_imoveis,
        (SELECT COUNT(*) FROM dados_imoveis WHERE possui_holding = FALSE AND possui_imoveis = TRUE) as potenciais_holding
    `);
