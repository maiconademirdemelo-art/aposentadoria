...
.modal{background:var(--surf);border:1px solid var(--bdr);border-radius:16px;width:100%;max-width:900px;margin:auto;overflow:hidden;animation:modalIn .25s ease}
...
<div class="modal-overlay" id="modal" onclick="fecharModal(event)">
  <div class="modal" id="modal-box">
    ...
    <div class="modal-actions">
      <button class="btn-del-modal" id="m-del-btn">🗑 Excluir diagnóstico</button>
...
function abrirModal(id){
  ...
  document.getElementById('m-sub').textContent = 'Diagnóstico #'+c.id+' · '+fmtDateLong(c.criado_em)+' · Posição #'+rank+' no ranking';
  ...
  document.getElementById('m-bd').textContent = ok
    ? 'Com '+fmtR(c.renda_mensal)+'/mês o patrimônio \${isRenda?"cresce indefinidamente":"se mantém por toda aposentadoria"}'
    : 'Considere aumentar o aporte ou ajustar a estratégia';

  // KPIs
  const anos = c.idade_aposentadoria - c.idade;
  ...
  document.getElementById('m-info').innerHTML = [
    ...
    {lbl:'Perfil Investidor',val:c.perfil.charAt(0).toUpperCase()+c.perfil.slice(1)},
    {lbl:'Estratégia',val:isRenda?'Viver de renda':'Consumir patrimônio'},
    {lbl:'Diagnóstico em',val:fmtDateLong(c.criado_em)},
    {lbl:'ID do Registro',val:'#'+c.id},
